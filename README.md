...
> Catatan: Google cukup sering mengubah tata letak menu di Firebase console. Kalau nama menu di bawah ini beda dengan yang kamu lihat, cari saja berdasarkan fitur (Firestore, Authentication, Service accounts) — bukan posisi menunya.

1. Ke [console.firebase.google.com](https://console.firebase.google.com) → **Add project**.
2. Di menu kiri: **Databases & Storage → Firestore → Create database** (pilih lokasi, mode production).
3. Di menu kiri: **Security → Authentication → Get started → Sign-in method → Anonymous → Enable**.
...
4. Sebelum push ke GitHub, buat file **`firebase-config.js`** di root project (isinya sama seperti `firebase-config.example.js`, tapi nilai-nilainya kamu ganti dengan config Firebase Web asli dari langkah 2.4). File ini memang ikut ter-commit ke GitHub — isinya public config, bukan rahasia (rahasianya ada di env var `FIREBASE_PRIVATE_KEY` yang terpisah).
5. **Deploy**.
